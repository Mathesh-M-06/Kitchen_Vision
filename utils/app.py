...
# 2. Safely import config
try:
    from config import settings
except ImportError:
    pass

# 3. Vision Service with fallback
try:
    from services.vision_services import VisionService
except ImportError:
    class VisionService:
        def detect_items(self, img_bytes) -> List[str]:
            return ["milk", "eggs", "flour", "apples"]
            
        def categorize_items(self, items) -> Dict[str, List[str]]:
            return {
...
# 4. Recipe Service with fallback
try:
    from services.recipe_service import RecipeService
except ImportError:
    class RecipeService:
        def generate_recipes(self, detected, categorized):
            return [{
                "title": "Simple Apple Pancakes",
                "difficulty": "Easy",
                "time_minutes": 20,
                "ingredients": ["milk", "eggs", "flour", "apples", "butter", "sugar"],
                "steps": [
                    "Dice the apples.",
                    "Mix milk, eggs, and flour to create a batter.",
                    "Fold in the diced apples.",
                    "Cook on a buttered skillet until golden brown."
                ]
            }]
            
        def identify_missing_ingredients(self, recipes, detected):
            # Simple mock logic: return ingredients from the first recipe not in detected
            needed = set(recipes[0]["ingredients"])
            have = set(detected)
            return list(needed - have)

# 5. Substitute Service with fallback
try:
...
# 6. Health Service with fallback
try:
